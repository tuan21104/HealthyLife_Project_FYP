...
class _DiaryScreenState extends State<DiaryScreen> {
    ...
    if (savedData != null) {
      try {
        ...
        setState(() {
          ...
          _targetCarb =
              (data['targetCarb'] as num?)?.toDouble() ??
              (_targetCalo * 0.5) / 4;
          _targetProtein =
              (data['targetProtein'] as num?)?.toDouble() ??
              (_targetCalo * 0.2) / 4;
          _targetFat =
              (data['targetFat'] as num?)?.toDouble() ??
              (_targetCalo * 0.3) / 9;
  ...
  Future<void> _saveDailyData() async {
    ...
    // 1. LƯU LOCAL (Nhanh như chớp để giao diện mượt)
    await prefs.setString(dateKey, jsonEncode(dataToSave));

    // 2. ĐỒNG BỘ LÊN CLOUD (Chạy ngầm ở background)
    // Tạm thời dùng 1 User ID giả định để test.
    // Sau này khi ráp Luồng Đăng nhập xong, bạn thay chuỗi này bằng ID của User thật nhé!
    String dummyUserId = "65f123456789abcde1234567";

    Map<String, dynamic> cloudData = {
      'userId': dummyUserId,
      'date': formattedDate,
      ...dataToSave, // Copy toàn bộ dữ liệu ở trên vào đây
    };

    // Bắn lên server (Không cần await để không làm chậm App)
  ...
  // --- HÀM SỬA MÓN ĂN (Tính lại theo tỷ lệ chéo) ---
  void _showEditFoodDialog(
    String mealType,
    int index,
    Map<String, dynamic> food,
  ) {
    // Tách "195 Gr" thành mảng ["195", "Gr"] để lấy số lượng cũ
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                if (newVal > 0 && oldVal > 0) {
                  double ratio = newVal / oldVal; // Tính tỷ lệ chênh lệch
                  setState(() {
  ...
  Future<void> _handleAddFood(String mealType) async {
    ...
    if (result != null) {
      setState(() {
        if (mealType == "Breakfast")
          _breakfastFoods.add(result);
        else if (mealType == "Lunch")
          _lunchFoods.add(result);
        else if (mealType == "Snack")
          _snackFoods.add(result);
        else if (mealType == "Dinner")
          _dinnerFoods.add(result);
      });
      _saveDailyData();
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              _buildMealSection("Exercise", [], _blueGreyColor),
              const SizedBox(height: 12),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      // --- NÂNG CẤP: DÙNG DISMISSIBLE VÀ GESTURE ĐỂ XÓA/SỬA MÓN ---
                      ...foods.asMap().entries.map((entry) {
                        ...
                        return Dismissible(
                          key:
                              UniqueKey(), // Cần Key độc nhất để Flutter biết đang vuốt dòng nào
                          direction: DismissDirection
                              .endToStart, // Chỉ cho vuốt từ phải sang trái
                          onDismissed: (direction) => _deleteFood(
                            title,
                            index,
                          ), // Gọi hàm xóa khi vuốt xong
                          ...
                          child: GestureDetector(
                            onTap: () => _showEditFoodDialog(
                              ...
                              index,
                              food,
                            ), // Bấm vào món ăn để sửa
                            child: Container(
                              color: Colors
                                  .transparent, // Bắt sự kiện Tap trên toàn bộ Row
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      food['kcal'].toStringAsFixed(1),
                                      style: const TextStyle(fontSize: 12),
                                    ),
                                  ),
                                  Expanded(
                                    child: Text(
                                      food['carb'].toStringAsFixed(1),
                                      style: const TextStyle(fontSize: 12),
                                    ),
                                  ),
                                  Expanded(
  ...
  // --- NÂNG CẤP: KHÓA MŨI TÊN CHỈ TRONG VÒNG 7 NGÀY ---
  Widget _buildDateSelector() {
    DateTime today = DateTime.now();
    DateTime todayOnly = DateTime(today.year, today.month, today.day);
    DateTime minDate = todayOnly.subtract(
      const Duration(days: 6),
    ); // Giới hạn lùi tối đa 6 ngày (tổng 7 ngày)
